...
namespace TimesheetManagement.Service.Areas.HelpPage
{
	...
	public class ObjectGenerator
	{
		...
		{
			private static readonly Dictionary<Type, Func<long, object>> DefaultGenerators = SimpleTypeObjectGenerator.InitializeGenerators();
			private long index;
			...
			{
				return SimpleTypeObjectGenerator.DefaultGenerators.ContainsKey(type);
			}

			public object GenerateObject(Type type)
			{
				return SimpleTypeObjectGenerator.DefaultGenerators[type](++this.index);
			}
		}
		...
		private object GenerateObject(Type type, Dictionary<Type, object> createdObjectReferences)
		{
			try
			{
				...
				if (type.IsArray)
				{
					return ObjectGenerator.GenerateArray(type, ObjectGenerator.DefaultCollectionSize, createdObjectReferences);
				}

				if (type.IsGenericType)
				{
					return ObjectGenerator.GenerateGenericType(type, ObjectGenerator.DefaultCollectionSize, createdObjectReferences);
				}

				if (type == typeof(IDictionary))
				{
					return ObjectGenerator.GenerateDictionary(typeof(Hashtable), ObjectGenerator.DefaultCollectionSize, createdObjectReferences);
				}

				if (typeof(IDictionary).IsAssignableFrom(type))
				{
					return ObjectGenerator.GenerateDictionary(type, ObjectGenerator.DefaultCollectionSize, createdObjectReferences);
				}
				...
				{
					return ObjectGenerator.GenerateCollection(typeof(ArrayList), ObjectGenerator.DefaultCollectionSize, createdObjectReferences);
				}

				if (typeof(IList).IsAssignableFrom(type))
				{
					return ObjectGenerator.GenerateCollection(type, ObjectGenerator.DefaultCollectionSize, createdObjectReferences);
				}

				if (type == typeof(IQueryable))
				{
					return ObjectGenerator.GenerateQueryable(type, ObjectGenerator.DefaultCollectionSize, createdObjectReferences);
				}
		...
		private static object GenerateGenericType(Type type, int collectionSize, Dictionary<Type, object> createdObjectReferences)
		{
			...
			if (genericArguments.Length == 1)
			{
				    ...
				    (genericTypeDefinition == typeof(ICollection<>)))
				{
					...
					return ObjectGenerator.GenerateCollection(collectionType, collectionSize, createdObjectReferences);
				}
			...
			if (genericArguments.Length == 2)
			{
				if (genericTypeDefinition == typeof(IDictionary<,>))
				{
					...
					return ObjectGenerator.GenerateDictionary(dictionaryType, collectionSize, createdObjectReferences);
				}
		...
		private static object GenerateQueryable(Type queryableType, int size, Dictionary<Type, object> createdObjectReferences)
		{
			...
			if (isGeneric)
			{
				...
				list = ObjectGenerator.GenerateCollection(listType, size, createdObjectReferences);
			}
			else
			{
				list = ObjectGenerator.GenerateArray(typeof(object[]), size, createdObjectReferences);
			}
			if (list == null)
			{
				return null;
		...
		private static object GenerateComplexObject(Type type, Dictionary<Type, object> createdObjectReferences)
		{
			...
			ObjectGenerator.SetPublicProperties(type, result, createdObjectReferences);
			ObjectGenerator.SetPublicFields(type, result, createdObjectReferences);
			return result;
		}
